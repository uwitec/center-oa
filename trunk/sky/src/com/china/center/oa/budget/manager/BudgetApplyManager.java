...
public class BudgetApplyManager
{
        ...
        throws MYException
    {
        ...
        if (nextStatus != BudgetConstant.BUDGET_APPLY_STATUS_END)
        {
            ...
            log(user, apply.getId(), OperationConstant.OPERATION_PASS, "通过预算变更");
        }
        else
        {
        ...
        log(user, apply.getBudgetId(), OperationConstant.OPERATION_PASS, "通过预算变更");
        ...
        throws MYException
    {
        ...
        for (BudgetItemBean budgetItemBean : items)
        {
            ...
            if (currentItem != null)
            {
                ...
                {
                    throw new MYException("预算项[%s]已经使用了%s,所以更改后的预算不能小于%s,您当前修改后的是%s",
            ...
            else
            {
                ...
                if (budget.getType() != BudgetConstant.BUDGET_TYPE_COMPANY)
                {
                    ...
                    if (parentItem == null)
                    {
                        throw new MYException("预算项[%s]是新增的,但是父预算没有此预算项",
                            currentItem.getFeeItemName());
                    }
                }
            }
        ...
        {
            throw new MYException("当前预算总额是%s,但是你变更后的是%s，不能大于当前总额请重新变更",
        ...
        throws MYException
    {
        ...
        for (BudgetItemBean budgetItemBean : items)
        {
            ...
            if (currentItem != null)
            {
                ...
                {
                    throw new MYException("预算项[%s]预算是%s,更改后的预算不能小于%s,您当前修改后的是%s",
                        currentItem.getFeeItemName(),
        ...
        {
            throw new MYException("当前预算总额是%s,但是你变更后的是%s，不能小于当前总额请重新追加",
                MathTools.formatNum(currentBudget), MathTools.formatNum(total));
        }
    }
