...
public class ProductExamineManager
{
    ...
    public boolean addBean(User user, ProductExamineBean bean, String[] stafferIdList)
        throws MYException
    {
     ...
     * @param item
     */
    private void createProductPlan(ProductCityExamineItemBean item, ProductExamineBean bean)
    {
     ...
     * @throws MYException
     */
    private void checkAdd(User user, ProductExamineBean bean, String[] stafferIdList)
        throws MYException
    {
        ...
        {
            StafferBean sbBean = stafferDAO.find(stafferId);
            ...
            ExamineBean examineBean = examineDAO.findByStafferIdAndYear(stafferId, financeYear);

            if (examineBean == null)
            {
                throw new MYException("当前系统内没有职员[%s]的个人考评,或者考评没有批准", sbBean.getName());
            }
            ...
            {
                throw new MYException("当前系统内职员[%s]的个人考评没有选择区域", sbBean.getName());
            }
            ...
            for (String cityId : list)
            {
                ...
                {
                    throw new MYException("区域[%s]缺少铺样配置", cityConfig.getCityName());
                }
                ...
                if (citySail == null)
                {
                    throw new MYException("区域[%d]铺样不存在", cityConfig.getCityName());
                }
                ...
                item.setPlanValue(citySail.getAmount() / citySail.getMonth() * bean.getMonth());
