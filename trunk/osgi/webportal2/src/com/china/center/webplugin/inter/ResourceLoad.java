...
public class ResourceLoad
{
    ...
    public static void init(final BundleContext context,
            final String webPluginName) throws Exception
    {
        ...
        new Thread()
        {
            public void run()
            {
                ...
                {
                    LOGGER.warn("webPluginName[" + webPluginName + "] in "
                            + USER_FLODER + ".So stop init.");
    ...
    private static void search(String pnane, Bundle bundle, String pattern,
            RegisterWebPlugin server)
    {
            ...
            for (Resource resource : patternResolver.getResources(pattern))
            {
                ...
                try
                {
                    ...
                    if (path != null && resource.exists())
                    {
                        ...
                        server.registerWebResource(pnane,
                                getPath(resource.getURI().toString()),
                                in);
                    }
                }
                catch (Exception e)
     ...
     * @throws Exception 
     */
    public static void destory(final BundleContext context,
            final String webPluginName) throws Exception
    {
        if (USER_FLODER.contains(webPluginName.toLowerCase()))
        {
            LOGGER.warn("webPluginName[" + webPluginName + "] in "
                    + USER_FLODER + ".So stop destory.");
