/**
 * File Name: OutStatDAOImpl.java<br>
...
public class OutStatDAOImpl extends BaseDAO<OutBean, OutBean> implements OutStatDAO
{
    ...
    public List<String> listCustomerIdList()
    {
        ...
        this.jdbcOperation.query("select distinct(t.customerId) from t_center_out t " + condtition.toString(),
        ...
        this.jdbcOperation.query("select distinct(t.cid) from t_center_vs_curcre t ", new RowCallbackHandler()
        {
            public void processRow(ResultSet rs)
                throws SQLException
            {
    ...
    public OutBean findNearestById(String id, String cid)
    {
        ...
        List<OutBean> list = this.jdbcOperation.queryObjects(
            "where 1= 1 and type = ? and outType = ? and customerId = ? and id < ? order by id desc", claz, 0, 0, cid,
            id).setMaxResults(1).list(claz);
    ...
    {
        this.jdbcOperation.update("set reserve1 = ? , reserve4 = ? where fullid = ?", claz, reserve1, reserve4, fullId);
    ...
    public double queryMaxBusiness(String cid, String beginDate, String endDate)
    {
        ...
        Object max = this.jdbcOperation.getIbatisDaoSupport().queryForObject("OutStatDAOImpl.queryMaxBusiness",
            paramterMap);
    ...
    public double sumBusiness(String cid, String beginDate, String endDate)
    {
        ...
        Object max = this.jdbcOperation.getIbatisDaoSupport().queryForObject("OutStatDAOImpl.sumBusiness", paramterMap);
    ...
    public List<NotPayWrap> listNotPayWrap()
    {
        ...
        List<NotPayWrap> result = this.jdbcOperation.getIbatisDaoSupport().queryForList(
            "OutStatDAOImpl.listNotPayWrap", paramterMap);
