       ...
       with(getEvent()){
          with(targetObject){
              ...
              if ((keyCode == 39 || keyCode == 37) && oldSel != '')
              {
	              ...
	              if (!window.event)
	              {
	              	  // ÓÉÓÚfirefoxÀïÃæµÄ×óÓÒ±»Õ¼ÓÃ£¬ËùÒÔÊÇ altKey + ×óÓÒ
	              	  if (!getEvent().altKey)
	              	  {
	              	  	  returnValue = false;
	              	      return;
              ...
              for(i=0;i<length;i++){
                  ...
                  if($Attribute(options[i], 'sp').indexOf(sel)==0){selectedIndex=i;break;}
              }
          }
...
function getEvent()
{
    ...
    while(func != null)
    {
    	//·µ»Øµ÷ÓÃº¯ÊýµÄ¡¾arguments.callee¡¿(calleeÊÇargumentsµÄÒ»¸öÊôÐÔÖµ)
    	//callee:·µ»ØÕý±»Ö´ÐÐµÄ Function ¶ÔÏó,ÕâÀï¾ÍÊÇfunc
    	//caller:·µ»ØÒ»¸ö¶Ôº¯ÊýµÄÒýÓÃ
