...
public class PageBodyCells extends BodyTagCenterSupport
{
	...
	public int doStartTag() throws JspException
	{
		...
		// 定义的每行CELL的数量
		int cells = ((Integer) pageContext.getAttribute(TagLibConstant.CENTER_CELLS_INIT + this.index))
				.intValue();

		// TR的索引值
		int trIndex = ((Integer) pageContext
				.getAttribute(TagLibConstant.CENTER_TRS_INDEX + this.index)).intValue();
	...
	private void writeStart(StringBuffer buffer)
	{
		...
		{
			buffer.append(
					"<td width='" + width + "%' align='" + align + "'" + temp + ">" + title
							+ "：</td>").append(line);
