...
<html>
<head>
<p:link title="配置新客户考核" cal="false" />
...
{
	submit('确定配置所有子考核的新客户考核?', null, null);
}
...
function refush()
{
	$l('../examine/examine.do?method=queryAllSubNewCustomerExamine&pid=${parentBean.id}');
}
...
<form name="formEntry" action="../examine/examine.do" method="post"><input
	type="hidden" name="method" value="configAllNewCustomerExamine"> <input type="hidden"
	name="pid" value="${parentBean.id}"> 
	<c:forEach items="${subs}" var="item">
        <input type="hidden" name="subItemId" value="${item.id}"> 
    </c:forEach>
	<p:navigation height="22">
	<td width="550" class="navigation">配置新客户考核</td>
	<td width="85"></td>
</p:navigation> <br>
...
<p:body width="100%">
	...
	<p:title>
		<td class="caption"><strong>【${parentBean.stafferName}的所有子考核】新客户考核信息：</strong></td>
	</p:title>
		...
		<table width="100%" align="center" cellspacing='1' class="table0"
			...
			<c:forEach items="${defaultList}" var="item" varStatus="vs">
				<tr class='${vs.index % 2 == 0 ? "content2" : "content1"}'>
					...
					<a title="点击查看详细" href="javaScript:void(0)" onclick="oopen('${subMap[subItem.id][vs.index].id}')">
